...
internal class DropTargetContainer(
    private val onDragStarted: (uris: List<File>, Offset) -> DragAction
) : Modifier.Element,
    ...
    override fun onDragStarted(files: List<File>, position: Offset): Boolean {
        coordinates ?: return false

        check(currentTarget == null)
        currentTarget = onDragStarted.invoke(files, position).target
...
//        activeChild = null
    ...
    override fun onDropped(files: List<File>, position: Offset): Boolean =
    ...
    override fun onDragEnded() {
        children.forEach { it.onDragEnded() }
